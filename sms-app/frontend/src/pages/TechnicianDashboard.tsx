...
import { FiAlertCircle, FiAlertTriangle, FiCheckCircle, FiCamera, FiClipboard, FiTruck, FiGrid, FiCalendar, FiMessageSquare, FiFileText, FiPackage, FiAlertOctagon, FiUsers, FiHeart, FiMessageCircle, FiCpu, FiPlus, FiClock, FiLogOut, FiPower, FiUser, FiEdit, FiSettings, FiX, FiChevronDown, FiEdit3, FiBook } from 'react-icons/fi';
import toast from 'react-hot-toast';
import TooltipButton from '../components/TooltipButton';
import DailyLog from '../components/DailyLog';
import HSEBoard from '../components/HSEBoard';
import TeamChatWidget from '../components/TeamChatWidget';
import TechnicianScorecard from '../components/TechnicianScorecard';
...
const TechnicianDashboard: React.FC = () => {
  ...
  const handleDirectFix = () => {
    navigate('/fault/direct-fix');
    toast.success('Direct Fix Entry Mode', { duration: 2000 });
  };
  ...
  const communityPosts = [
    {
      ...
      content: 'Pro tip: When troubleshooting VFD faults on the mud pumps, always check the DC bus voltage first. Saved me 2 hours yesterday!',
      likes: 23,
      replies: 5,
      time: '3 hours ago',
      tags: ['electrical', 'mud-pumps', 'troubleshooting']
    },
    {
      id: 2,
      ...
      content: 'Anyone else experiencing intermittent ground faults on the SCR systems? Started after the last software update.',
      likes: 15,
      replies: 12,
      time: '8 hours ago',
      tags: ['scr', 'ground-fault', 'software']
    },
    {
      id: 3,
      ...
      content: 'Just completed overhaul of Draw Works motor. Documentation and photos in the shared drive. Hit me up if you need the torque specs!',
      likes: 34,
      replies: 8,
      time: 'Yesterday',
      tags: ['draw-works', 'maintenance', 'documentation']
    }
  ];

  return (
    <div className={`min-h-screen ${theme === 'dark' ? 'bg-gradient-to-br from-sms-dark to-sms-gray' : 'bg-gradient-to-br from-gray-100 to-gray-200'}`}>
      {/* Header */}
      <div className={`${theme === 'dark' ? 'bg-sms-dark/80 border-gray-700' : 'bg-white/80 border-gray-300'} backdrop-blur-sm border-b sticky top-0 z-40`}>
        <div className="max-w-7xl mx-auto px-4 py-3">
          <div className="flex justify-between items-center">
            <div className="flex items-center space-x-4">
              <div>
                <h1 className={`text-xl font-bold ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>Technician Dashboard</h1>
                <div className="flex items-center space-x-2">
                  <p className={`text-sm ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>{user?.firstName} {user?.lastName} • Drilling Electrician</p>
      ...
      <div className="max-w-7xl mx-auto p-4">
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            ...
            {/* Direct Fix Entry - Green */}
            <div className="relative group">
              <button
                onClick={handleDirectFix}
                className="relative overflow-hidden bg-green-900/20 border border-green-500/30 hover:border-green-500 rounded-xl p-4 transition-all duration-300 hover:scale-105 hover:shadow-lg hover:shadow-green-500/20 w-full"
              >
                <div className="absolute inset-0 bg-gradient-to-r from-green-600/0 via-green-600/10 to-green-600/0 transform -skew-x-12 -translate-x-full group-hover:translate-x-full transition-transform duration-700"></div>
                <FiCheckCircle className="w-8 h-8 text-green-500 mx-auto mb-2" />
                <p className="text-white font-semibold text-sm">Direct Fix</p>
                <p className="text-xs text-green-400 mt-1">Post-Fix Entry</p>
              </button>
              <div className="absolute -bottom-12 left-0 bg-sms-dark border border-green-500/30 rounded-lg px-3 py-2 opacity-0 group-hover:opacity-100 transition-opacity z-50 pointer-events-none min-w-max">
                <p className="text-xs text-green-400 font-semibold">Already Fixed Something?</p>
                <p className="text-xs text-gray-400">Record completed repairs & parts used</p>
              </div>
            </div>
            ...
            <div className="relative group">
              <button
                onClick={() => navigate('/handover/complete')}
                className={`relative overflow-hidden border rounded-xl p-4 transition-all duration-300 hover:scale-105 hover:shadow-lg w-full ${
                  handoverProgress === 100 
                    ? 'bg-green-900/20 border-green-500/30 hover:border-green-500 hover:shadow-green-500/20' 
                    : handoverProgress >= 60 
                      ? 'bg-amber-900/20 border-amber-500/30 hover:border-amber-500 hover:shadow-amber-500/20'
                      : 'bg-sms-gray border-gray-600 hover:border-blue-500 hover:shadow-blue-500/20'
                }`}
              >
                <div className={`absolute inset-0 bg-gradient-to-r transform -skew-x-12 -translate-x-full group-hover:translate-x-full transition-transform duration-700 ${
                  handoverProgress === 100 
                    ? 'from-green-600/0 via-green-600/10 to-green-600/0' 
                    : handoverProgress >= 60 
                      ? 'from-amber-600/0 via-amber-600/10 to-amber-600/0'
                      : 'from-blue-600/0 via-blue-600/10 to-blue-600/0'
                }`}></div>
                <div className="relative">
                  <FiClipboard className={`w-8 h-8 mx-auto mb-2 ${
                    handoverProgress === 100 ? 'text-green-500' : 
                    handoverProgress >= 60 ? 'text-amber-500' : 'text-blue-400'
                  }`} />
                  {handoverProgress > 0 && handoverProgress < 100 && (
                    <div className="absolute -top-1 -right-1 w-6 h-6 bg-sms-dark rounded-full flex items-center justify-center">
                      <span className="text-xs font-bold text-white">{handoverProgress}%</span>
                    </div>
                  )}
                </div>
                <p className="text-white font-semibold text-sm">Handover</p>
                <p className={`text-xs mt-1 ${
                  handoverProgress === 100 ? 'text-green-400' : 
                  handoverProgress >= 60 ? 'text-amber-400' : 'text-gray-400'
                }`}>
                  {handoverProgress === 100 ? 'Completed' : 
                   handoverProgress > 0 ? `${handoverProgress}% Done` : 'Shift Notes'}
                </p>
              </button>
              <div className="absolute -bottom-12 right-0 bg-sms-dark border border-gray-600 rounded-lg px-3 py-2 opacity-0 group-hover:opacity-100 transition-opacity z-50 pointer-events-none min-w-max">
                <p className={`text-xs font-semibold ${
                  handoverProgress === 100 ? 'text-green-400' : 
                  handoverProgress >= 60 ? 'text-amber-400' : 'text-blue-400'
                }`}>
                  {handoverProgress === 100 ? 'Handover Complete!' : 'Complete Handover'}
                </p>
                <p className="text-xs text-gray-400">
                  Fill out the handover form for next tech
                </p>
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="bg-red-900/20 border border-red-500/30 rounded-lg p-3">
                <div className="flex items-start justify-between">
                  <div className="flex items-start space-x-3">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-red-400">OVERDUE: HPU 1 Motor</p>
                      <p className="text-xs text-gray-400">Quarterly inspection - 3 days overdue</p>
                    </div>
                  </div>
                  <button
                    onClick={() => navigate('/equipment/hpu-1-motor')}
              ...
              <div className="bg-amber-900/20 border border-amber-500/30 rounded-lg p-3">
                <div className="flex items-start justify-between">
                  <div className="flex items-start space-x-3">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-amber-400">DUE SOON: Mud Pump 2</p>
                      <p className="text-xs text-gray-400">Annual service - Due in 2 days</p>
                    </div>
                  </div>
                  <button
                    onClick={() => navigate('/equipment/mud-pump-2')}
              ...
              <div className="bg-amber-900/20 border border-amber-500/30 rounded-lg p-3">
                <div className="flex items-start justify-between">
                  <div className="flex items-start space-x-3">
                    <div className="w-2 h-2 bg-amber-500 rounded-full mt-2"></div>
                    <div>
                      <p className="text-sm font-semibold text-amber-400">DUE SOON: Generator 3</p>
                      <p className="text-xs text-gray-400">Oil change - Due in 5 days</p>
                    </div>
                  </div>
                  <button
                    onClick={() => navigate('/equipment/gen-3')}
                    className="text-xs text-amber-400 hover:text-amber-300 whitespace-nowrap"
                  >
                    Go to Equipment →
                  </button>
              ...
              <div className="bg-gray-800 border border-gray-600 rounded-lg p-3">
                <div className="flex items-center space-x-2 text-gray-400">
                  ...
                  <p className="text-xs">12 more maintenance tasks scheduled this month</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-3">
              ...
              <div className="bg-red-900/20 border border-red-500/30 rounded-lg p-3">
                <div className="flex items-start justify-between">
                  <div className="flex items-start space-x-3">
                    ...
                    <div>
                      <div className="flex items-center space-x-2">
                        <p className="text-sm font-semibold text-red-400">CRITICAL: Thruster 2 Failure</p>
                        <span className="text-xs text-red-300 bg-red-900/50 px-2 py-0.5 rounded">2h 15m</span>
                      </div>
                      <p className="text-xs text-gray-400">Assigned: Mike Chen • Status: Diagnostics in progress</p>
                    </div>
                  </div>
                  <button
              ...
              <div className="bg-amber-900/20 border border-amber-500/30 rounded-lg p-3">
                <div className="flex items-start justify-between">
                  <div className="flex items-start space-x-3">
                    ...
                    <div>
                      <div className="flex items-center space-x-2">
                        <p className="text-sm font-semibold text-amber-400">MINOR: SCR Panel Warning</p>
              ...
              <div className="bg-amber-900/20 border border-amber-500/30 rounded-lg p-3">
                <div className="flex items-start justify-between">
                  <div className="flex items-start space-x-3">
                    ...
                    <div>
                      <div className="flex items-center space-x-2">
                        <p className="text-sm font-semibold text-amber-400">MINOR: Oil Leak - HPU 2</p>
                        <span className="text-xs text-amber-300 bg-amber-900/50 px-2 py-0.5 rounded">3d</span>
                      </div>
                      <p className="text-xs text-gray-400">Assigned: Tom R. • Status: Monitoring</p>
                    </div>
                  </div>
                  <button
                    onClick={() => navigate('/equipment/hpu-2-motor')}
                    className="text-xs text-amber-400 hover:text-amber-300 whitespace-nowrap"
                  >
                    View →
                  </button>
              ...
              <div className="bg-gray-800 border border-gray-600 rounded-lg p-3">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="text-xs text-gray-500">Avg Resolution: 4.2h</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="mb-6">
          <TechnicianScorecard technicianId={user?.id || 0} compact={true} />
        </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="relative group">
                ...
                >
                  <div>
                    ...
                    <p className="text-gray-400 text-sm">Drilling equipment</p>
                  </div>
                  <div className="flex justify-between items-center mt-3">
                    <span className="text-xs text-gray-500">47 Units</span>
                    <span className="text-xs text-green-400">3 Due</span>
                  </div>
                </div>
                <div className="absolute -bottom-16 left-1/2 transform -translate-x-1/2 bg-sms-dark border border-sms-cyan/30 rounded-lg px-4 py-3 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-50 pointer-events-none">
                  ...
                  <p className="text-xs text-gray-500 mt-1">Organized by: SCR, Mud Pumps, Top Drive, etc.</p>
                </div>
              </div>
              ...
              <div className="relative group">
                ...
                >
                  ...
                  <div className="flex justify-between items-center mt-3">
                    <span className="text-xs text-gray-500">Week: 8 Tasks</span>
                    <span className="text-xs text-amber-400">2 Overdue</span>
                  </div>
                </div>
              ...
              <div className="relative group">
                ...
                >
                  ...
                  <div className="flex justify-between items-center mt-3">
                    <span className="text-xs text-gray-500">847 Records</span>
              ...
              <div className="relative group">
                ...
                >
                  ...
                  <div className="flex justify-between items-center mt-3">
                    <span className="text-xs text-gray-500">341 Items</span>
                    <span className="text-xs text-red-400">5 Low Stock</span>
                  </div>
                </div>
              ...
              <div className="space-y-3 pr-2">
                ...
                <div className="bg-purple-800/20 rounded-lg p-3">
                  ...
                  <ul className="space-y-2 text-xs text-gray-300">
                    <li className="flex items-start space-x-2">
                      ...
                      <span>HPU 1 Motor showing early signs of bearing wear - schedule inspection within 2 weeks</span>
                    </li>
                    <li className="flex items-start space-x-2">
                      <span className="text-red-400">•</span>
                      <span>VFD Panel 2 temperature trending high - check cooling system today</span>
                    </li>
                  </ul>
                </div>
                ...
                <div className="bg-purple-800/20 rounded-lg p-3">
                  ...
                  <ul className="space-y-2 text-xs text-gray-300">
                    <li className="flex items-start space-x-2">
                      ...
                      <span>Complete SCR calibration during downtime - estimated 45 mins</span>
                    </li>
                    <li className="flex items-start space-x-2">
                      <span className="text-blue-400">•</span>
                      <span>Ideal time for Top Drive grease cartridge replacement based on usage</span>
                    </li>
                  </ul>
                </div>
                ...
                <div className="bg-purple-800/20 rounded-lg p-3">
                  ...
                  <ul className="space-y-2 text-xs text-gray-300">
                    <li className="flex items-start space-x-2">
                      ...
                      <span>Mud Pump liner stock low (2 remaining) - reorder recommended</span>
                    </li>
                    <li className="flex items-start space-x-2">
                      <span className="text-green-400">•</span>
                ...
                <div className="bg-purple-900/30 border border-purple-500/20 rounded-lg p-3">
                  <p className="text-sm text-purple-200 italic">
                    "Based on fleet-wide data, equipment with your VFD model typically requires DC bus capacitor replacement at 18,000 hours. Your unit is at 17,850."
                  </p>
        ...
        {/* HSE Board */}
        <div className="mt-8">
          <HSEBoard vesselId={selectedVessel || undefined} isCompact={true} />
        </div>
        ...
        <div className="mt-8">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center space-x-3">
              ...
              <span className="text-xs bg-sms-cyan/20 text-sms-cyan px-2 py-1 rounded-full">12 New</span>
            </div>
            <div className="relative group">
              <button
